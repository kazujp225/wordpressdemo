...
import type { ThumbnailListItem } from '@/types/thumbnail';
...
export function ThumbnailCard({ thumbnail, onDelete }: ThumbnailCardProps) {
    const [showDeleteConfirm, setShowDeleteConfirm] = useState(false);
    const deleteApi = useApiMutation();

    const handleDelete = async () => {
        await deleteApi.execute({
            endpoint: `/api/thumbnails/${thumbnail.id}`,
            method: 'DELETE',
            successMessage: 'サムネイルを削除しました',
    ...
    const categoryConfig = THUMBNAIL_CATEGORIES.find((c) => c.id === thumbnail.category);
    const imagePath = thumbnail.image?.filePath;

    return (
        <div className="group overflow-hidden rounded-lg border border-border bg-background transition-all hover:border-primary/50">
            <div className="aspect-[16/9] overflow-hidden border-b border-border bg-surface-50 relative">
                <Link
                ...
                <div className="absolute top-2 left-2 z-20">
                    <span className="rounded-sm bg-primary/90 px-2 py-1 text-[10px] font-bold text-white">
                        {categoryConfig?.shortName || thumbnail.category}
                    </span>
                </div>
                <div className="absolute top-2 right-2 z-20">
                    <span className={clsx(
                        'rounded-sm px-2 py-1 text-[10px] font-bold uppercase tracking-widest',
                        thumbnail.status === 'saved'
                            ? 'bg-green-500 text-white'
                            : thumbnail.status === 'generated'
                            ? 'bg-blue-500 text-white'
                            : 'bg-muted text-muted-foreground'
                    )}>
                        {thumbnail.status === 'saved' ? 'Saved' : thumbnail.status === 'generated' ? 'Generated' : 'Draft'}
                    </span>
                </div>
                {imagePath ? (
            ...
            <div className="p-3 sm:p-5">
                <div className="overflow-hidden">
                    ...
                    <p className="text-xs text-muted-foreground mt-1 font-mono">
                        ...
                        {thumbnail.presetName && (
                            <span className="ml-2 text-muted-foreground/70">({thumbnail.presetName})</span>
                        )}
                    </p>
                </div>
                <div className="mt-4 flex items-center justify-between border-t border-border pt-3">
                    <span className="text-[10px] font-bold text-muted-foreground uppercase tracking-wider">
                        Updated: {formatDate(thumbnail.updatedAt)}
                    </span>
                    {showDeleteConfirm ? (
