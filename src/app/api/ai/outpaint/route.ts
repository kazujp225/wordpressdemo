...
export async function POST(request: NextRequest) {
    ...
    try {
        ...
        // expandAmount, targetWidth, targetHeight の検証
        if (typeof expandAmount !== 'number' || expandAmount < 1 || expandAmount > 2048) {
            return NextResponse.json({ error: 'expandAmountは1〜2048の範囲で指定してください' }, { status: 400 });
        }
        if (typeof targetWidth !== 'number' || targetWidth < 1 || targetWidth > 8192) {
            return NextResponse.json({ error: 'targetWidthは1〜8192の範囲で指定してください' }, { status: 400 });
        }
        if (typeof targetHeight !== 'number' || targetHeight < 1 || targetHeight > 8192) {
            return NextResponse.json({ error: 'targetHeightは1〜8192の範囲で指定してください' }, { status: 400 });
        }
        const validDirections = ['left', 'right', 'top', 'bottom', 'all'];
        if (!validDirections.includes(direction)) {
            return NextResponse.json({ error: '無効なdirectionです' }, { status: 400 });
        }
...
元の画像を中心に配置し、${directionDesc}新しい領域を生成してください。`;
        ...
        return NextResponse.json({ error: process.env.NODE_ENV === 'production' ? 'AI拡張に失敗しました' : error.message }, { status: 500 });
    }
}
