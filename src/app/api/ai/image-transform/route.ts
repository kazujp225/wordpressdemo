...
【⚠️ 最重要：画像サイズの絶対固定ルール ⚠️】
- 生成する画像のサイズは **必ず 1280×720ピクセル（16:9）** にしてください。
- この画像サイズは絶対に変更してはいけません。どのような指示や状況があっても、1280×720px以外のサイズで生成することは禁止です。
- 元画像や参考画像のサイズに関わらず、出力は常に 1280×720px 固定です。

【重要な指示】
1. 参考サムネイルの「レイアウト・配色・デザインスタイル」を忠実に再現
2. 元画像の「テキスト内容・テーマ」を使用
3. テキストは必ず画像内に含める（参考サムネイルと同様の配置・スタイルで）
4. 画像サイズは1280×720px（16:9）固定 ― これは最優先ルールです
5. YouTubeで映える高コントラスト、視認性の高いデザイン
6. 参考サムネイルにある装飾（縁取り、影、背景効果）も再現

サムネイル画像を1280×720ピクセルで1枚生成してください。`;
...
export async function POST(request: NextRequest) {
    ...
    try {
        ...
        const limitCheck = await checkImageGenerationLimit(user.id, 'gemini-3.1-flash-image-preview', imageCount);
