...
### Gemini API 料金 (2026年3月時点)

| モデル | タイプ | 料金 |
|--------|--------|------|
| `gemini-3.1-flash-image-preview` | 画像生成 1K | **$0.067/画像** (~10円) |
| `gemini-3.1-flash-image-preview` | 画像生成 2K | **$0.101/画像** (~15円) |
| `gemini-3.1-flash-image-preview` | 画像生成 4K | **$0.151/画像** (~23円) |
| `gemini-2.0-flash` | テキスト | $0.10/100万入力トークン + $0.40/100万出力トークン |

**画像出力トークン単価**: $60/100万トークン（1K=1,120tokens, 2K=1,680tokens, 4K=2,520tokens）
...
| **モデル** | `real-esrgan-thick` (ローカル) / `gemini-3.1-flash-image-preview` (フォールバック) |
| **コスト** | **無料** (Real-ESRGAN) / $0.067 (AIフォールバック時、1K) |
| **用途** | 画像の高解像度化 |
...
2. 画像再生成: $0.067 × セクション数
---
合計: 10セクションで約 $0.68 (~102円)
```

### カラーパレット適用 → 再生成 フロー（1K解像度）
```
1. デザイン解析: ~$0.001
2. 画像再生成: $0.067 × セクション数
---
合計: 10セクションで約 $0.67 (~101円)
```

### 新規LP作成（フル、1K解像度）
```
1. LP生成: $0.067 × 8セクション = $0.54
2. コピー生成: ~$0.008
3. ナビ生成: ~$0.0003
---
合計: 約 $0.55 (~83円)
...
### URLインポート → リスタイル（1K解像度）
```
1. URLインポート: $0.067 × 10セグメント = $0.67
2. デザイン解析: ~$0.001
3. リスタイル: $0.067 × 10セクション = $0.67
---
合計: 約 $1.34 (~201円)
```
...
*最終更新: 2026年3月（gemini-3.1-flash-image-preview移行）*
